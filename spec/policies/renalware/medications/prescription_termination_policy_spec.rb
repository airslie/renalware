module Renalware
  module Medications
    describe PrescriptionTerminationPolicy, type: :policy do
      ...
      let(:termination) { build(:prescription_termination, prescription: prescription) }
      ...
      %i(new? create?).each do |permission|
        permissions permission do
          context "when user has prescriber role but no clinical/admin/superadmin/devops" do
            ...
            it { is_expected.not_to permit(user, termination) }
          end
        ...
        permissions permission do
          context "when clinical user has prescriber role" do
            let(:user) { user_double_with_role(%i(clinical prescriber)) }

            it { is_expected.to permit(user, termination) }
          end
